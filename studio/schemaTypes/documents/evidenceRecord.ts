...
export const evidenceRecord = defineType({
  ...
  fields: [
    defineField({name: 'evidenceType', title: 'Evidence type', type: 'string', group: 'media', options: {list: [{title: 'Image', value: 'IMAGE'}, {title: 'Video', value: 'VIDEO'}, {title: 'Certificate', value: 'CERTIFICATE'}], layout: 'radio'}, validation: (Rule) => Rule.required()}),
    ...
    defineField({name: 'certificateName', title: 'Certificate name', type: 'string', group: 'media', hidden: ({parent}) => parent?.evidenceType !== 'CERTIFICATE'}),
    defineField({name: 'certificateHolder', title: 'Certificate holder / legal entity', type: 'string', group: 'media', hidden: ({parent}) => parent?.evidenceType !== 'CERTIFICATE'}),
    defineField({name: 'certificateIssuer', title: 'Issuer', type: 'string', group: 'media', hidden: ({parent}) => parent?.evidenceType !== 'CERTIFICATE'}),
    defineField({name: 'certificateNumber', title: 'Certificate number', type: 'string', group: 'media', hidden: ({parent}) => parent?.evidenceType !== 'CERTIFICATE'}),
    defineField({name: 'certificateScope', title: 'Certificate scope', type: 'text', rows: 2, group: 'media', hidden: ({parent}) => parent?.evidenceType !== 'CERTIFICATE'}),
    defineField({name: 'certificateIssuedDate', title: 'Issued date', type: 'date', group: 'media', hidden: ({parent}) => parent?.evidenceType !== 'CERTIFICATE'}),
    defineField({name: 'certificateExpiryDate', title: 'Expiry date', type: 'date', group: 'media', hidden: ({parent}) => parent?.evidenceType !== 'CERTIFICATE'}),
    defineField({name: 'certificateFile', title: 'Verifiable certificate file', type: 'file', group: 'media', hidden: ({parent}) => parent?.evidenceType !== 'CERTIFICATE'}),
    defineField({name: 'offeringRelationship', title: 'Relationship to POXIOL offering', type: 'text', rows: 2, group: 'media', hidden: ({parent}) => parent?.evidenceType !== 'CERTIFICATE'}),
    ...
    defineField({name: 'sampleId', title: 'Related sample ID', type: 'string', group: 'relations'}),
    defineField({name: 'authorizationStatus', title: 'Project/public authorization', type: 'string', group: 'verification', options: {list: [
