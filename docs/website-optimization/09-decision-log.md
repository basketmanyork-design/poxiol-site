...
| Use static country labels in the project form | A local browser reproduced a React hydration mismatch caused by browser/server locale data disagreeing on a country name. Static labels remove that runtime dependency without changing the country code set. |
| Keep the Hero clear of mobile fixed controls | The mobile inquiry bar now leaves the Hero viewport. The analytics panel shifts to the safe-area edge when that bar is absent and compacts at 320px, so both Hero CTAs remain usable. |
