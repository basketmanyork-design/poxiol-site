...
export async function WhatsAppButton() {
  ...
  return (
    <a
      ...
      aria-label={`Chat with ${chrome.brandName} on WhatsApp`}
    >
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-white/10 bg-neutral-950/95 backdrop-blur-xl">
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 md:px-10 xl:px-20">
        <Link href="/" className="text-2xl font-black tracking-tight text-white uppercase">{chrome.brandName}<span className="text-[#B6FF00]">.</span></Link>
        ...
        <Link href={freeMockupHref} className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/15 text-white lg:hidden" aria-label="Request free mockup">→</Link>
      </div>
    </header>
  );
}
