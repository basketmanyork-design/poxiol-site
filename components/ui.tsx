...
export function SectionHeading({ eyebrow, title, subtitle, dark = false }: { eyebrow: string; title: string; subtitle?: string; dark?: boolean }) {
  return (
    <div className="mx-auto mb-12 max-w-3xl text-left md:mb-14 md:text-center">
      <p className="mb-4 text-sm font-extrabold uppercase tracking-[0.16em] text-[#B6FF00]">{eyebrow}</p>
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-white/10 bg-neutral-950/95 backdrop-blur-xl">
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 md:px-10 xl:px-20">
        <Link href="/" className="text-2xl font-black tracking-tight text-white">POXIOL<span className="text-[#B6FF00]">.</span></Link>
        <nav className="hidden items-center gap-8 lg:flex">{nav.map(([label, href]) => <Link key={label} href={href} className="text-sm font-bold text-white/80 transition hover:text-[#B6FF00]">{label}</Link>)}</nav>
        <Link href="/free-mockup/" className="hidden rounded-full bg-[#B6FF00] px-6 py-3 text-sm font-black text-neutral-950 transition hover:bg-white md:inline-flex">GET FREE MOCKUP</Link>
    ...
    <footer className="border-t border-white/10 bg-neutral-950 px-5 py-16 text-white md:px-10 xl:px-20">
      <div className="mx-auto max-w-7xl">
        <div className="grid gap-10 lg:grid-cols-[1.4fr_repeat(3,1fr)]">
          <div><Link href="/" className="text-3xl font-black">POXIOL<span className="text-[#B6FF00]">.</span></Link><p className="mt-4 max-w-xs leading-7 text-neutral-400">Custom sports uniforms for clubs, schools, brands and teams worldwide.</p><div className="mt-6"><PrimaryButton>Get Free Mockup</PrimaryButton></div></div>
          {columns.map(col => <div key={col.title}><h3 className="font-black text-white">{col.title}</h3><ul className="mt-4 space-y-3">{col.links.map(([label, href]) => <li key={label}><Link href={href} className="text-sm text-neutral-400 transition hover:text-[#B6FF00]">{label}</Link></li>)}</ul></div>)}
        </div>
