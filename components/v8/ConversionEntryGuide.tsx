    ...
    <section className="border-y border-neutral-200 bg-neutral-50 px-5 py-12 text-neutral-950 md:px-10 md:py-16 xl:px-20" aria-labelledby="conversion-entry-guide-title">
      <div className="mx-auto max-w-7xl">
        <div className="max-w-3xl">
          ...
          <p className="mt-4 leading-7 text-neutral-600">Use the entry that matches your current purchasing stage. All routes use the same secure project review workflow.</p>
        </div>
          ...
          {V8_CONVERSION_ENTRIES.map((entry) => {
            ...
            return (
              <article key={entry.intent} className={`rounded-2xl border p-5 ${active ? 'border-lime-400 bg-lime-400/10' : 'border-neutral-200 bg-white'}`}>
                ...
                {active ? <p className="mt-4 text-xs font-black uppercase tracking-widest text-lime-700">Current Path</p> : <Link href={entry.path} className="mt-4 inline-flex min-h-11 items-center text-xs font-black uppercase tracking-widest text-neutral-950 underline decoration-lime-400 decoration-2 underline-offset-4">Use This Path</Link>}
              </article>
            )
          })}
        </div>
