    ...
    <section className="bg-neutral-950 px-5 pb-28 pt-4 text-white sm:pt-12 md:px-10 md:pb-16 md:pt-10 xl:px-20" aria-labelledby={headingId}>
      <div className={`mx-auto grid w-full min-w-0 max-w-7xl items-center gap-10 ${hasVisual ? 'lg:grid-cols-[1.2fr_0.8fr] lg:gap-14' : ''}`}>
        <div className="min-w-0">
          ...
          <p className="mt-6 max-w-full break-words [overflow-wrap:anywhere] text-base leading-8 text-neutral-300 sm:max-w-2xl md:text-lg">{config.description}</p>
          <div className="mt-6 flex w-full min-w-0 flex-col gap-2 sm:mt-8 sm:flex-row sm:gap-3">
            <PrimaryButton href={primary.href} className="min-h-14 w-full min-w-0 sm:flex-1 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#B6FF00]">{primary.label}</PrimaryButton>
