...
import {readJsonWithRetry} from '../sanity/read-retry.mjs'
...
export async function buildSanityEvidence({candidateCommit, reads, fetchImpl = fetch, sleepImpl}) {
  if (!COMMIT_PATTERN.test(candidateCommit || '')) throw new Error('SANITY_BUILD_EVIDENCE_INVALID_COMMIT')
  ...
  for (const read of reads) {
    ...
    const readResult = await readJsonWithRetry(
      url,
      {method: 'GET', headers: {Accept: 'application/json'}},
      {fetchImpl, ...(sleepImpl ? {sleepImpl} : {})},
    )
    if (!readResult.ok) {
      if (readResult.failure === 'http') {
        throw new Error(`SANITY_BUILD_EVIDENCE_HTTP:${read.name}:${readResult.status}`)
      }
      if (readResult.failure === 'json') throw new Error(`SANITY_BUILD_EVIDENCE_JSON:${read.name}`)
