...
export async function getSiteChrome(): Promise<CmsSiteChrome> {
  return resolveContent(async () => {
    ...
    return {
      ...
      whatsappNumber,
      whatsappHref: digits ? `https://wa.me/${digits}` : legacySiteChrome.whatsappHref,
      whatsappMessage: settings?.contactInfo?.whatsappMessage || legacySiteChrome.whatsappMessage,
