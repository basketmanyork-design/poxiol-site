...
export function normalizeBuyerFacingClaim(value: string): string {
  return value
    .replace(/\bfactory[-\s]+direct\b/gi, 'teamwear manufacturing partner')
    .replace(/\bdirect\s+(?:manufacturer|factory)\b/gi, 'teamwear manufacturing partner')
    .replace(/\bunlimited\s+teamwear\s+customization\s+options\b/gi, 'Project-Specific Teamwear Customization Options')
    .replace(/\bunlimited\s+(?:customization|colou?rs?|designs?|design\s+options?|options?)\b/gi, 'project-specific customization options')
    ...
    .replace(/\b(?:production\s+capacity\s*:?\s*)?\d[\d,]*\+?\s+units(?:\s+(?:per\s+month|monthly))?(?:\s+production\s+capacity)?\b\.?/gi, 'Production scheduling is confirmed after project review.')
    .replace(/\b(?:production|factory)\s+capacity\b/gi, 'production schedule')
    .replace(/\b100%\s+polyester\b/gi, 'polyester')
    .replace(/\b100%\s+(?:manual\s+)?QC\b/gi, 'documented QC')
    .replace(/\b100%\s*/g, '')
    ...
    .replace(/Elite\s+B2B\s+custom\s+teamwear\s+manufacturer/gi, 'custom teamwear manufacturing partner')
