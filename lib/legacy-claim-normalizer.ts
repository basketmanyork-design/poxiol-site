...
export function normalizeBuyerFacingClaim(value: string): string {
  return value
    ...
    .replace(/POXIOL\s+is\s+a\s+factory[- ]direct\s+custom\s+teamwear\s+manufacturer/gi, 'POXIOL is a brand operated by Quanzhou Lanren Electronic Commerce Co., Ltd. It supports B2B custom teamwear projects')
    .replace(/POXIOL\s+provides\s+factory[- ]direct\s+custom\s+teamwear\s+manufacturing/gi, 'POXIOL supports B2B custom teamwear projects')
    .replace(/POXIOL\s+coordinates\s+factory[- ]direct\s+custom\s+teamwear/gi, 'POXIOL coordinates custom teamwear')
