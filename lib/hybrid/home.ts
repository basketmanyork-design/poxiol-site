...
const limitation = 'Owner-approved editorial wording only; it does not prove execution, capacity, delivery, customer outcomes, or current availability. Final feasibility remains project-specific.'

export const hybridHome = {
  provenance: {
    source: 'Approved local projection: site/content/approved/pages/home.json and claims.json',
    status: 'EDITORIAL_REVIEW_PENDING',
    note: 'This local source projection is not CMS publication. Legacy detail-page integration remains later work.',
  },
  seo: {
  ...
  hero: {title: 'Custom Teamwear Built for Repeatable Team Orders', body: 'POXIOL supports teamwear distributors, dealers, sportswear brands, and custom resellers serving teams worldwide with a controlled path from design review through repeat orders.', limitation},
  audience: {title: 'For Teamwear Distributors, Dealers, Brands & Custom Resellers', body: 'Six risks can turn a team order into a client problem. Our process is built to control them.', limitation},
  risks: [
    {title:'Design Accuracy', body:'Complex customization, confirmed before production.', limitation, href:'/customization/', cta:'Review customization'},
    {title:'Size & Fit', body:'Team sizing is checked before the order is locked.', limitation, href:'/guides/basketball-uniform-size-guide/', cta:'Basketball size guide'},
    {title:'Project Deadline', body:'Plan backward from the date your customer needs the order.', limitation, href:'/shipping-after-sales/', cta:'Review shipping planning'},
    {title:'Sample-to-Bulk', body:'Use the approved sample as the bulk reference.', limitation, href:'/guides/teamwear-sample-approval-checklist/', cta:'Review sample checklist'},
    {title:'Reorder Consistency', body:'Reorders should start from confirmed records—not memory.', limitation, href:'/get-quote/?product=Teamwear%20Reorder&source=%2F', cta:'Discuss a reorder'},
    {title:'Account Expansion', body:'Start with one team order. Expand as the account grows.', limitation, href:'/private-label-teamwear/', cta:'Review private-label options'},
  ],
  capabilities: [
  ...
  ].map(([title, body]) => ({title, body, limitation})),
  explanations: [
  ...
  closing: {title:'Tell us where the risk is before we quote the project.', body:'Share your product, quantity, design status, size mix, destination and target in-hand date. We’ll help you define the right approval path before production.', limitation},
  intents: {
