...
const corePageFallbacks: Record<string, {title: string; eyebrow: string; heading: string; description: string; seoTitle: string; metaDescription: string; image?: {url: string; alt: string}}> = {
  ...
  'get-quote': {title: 'Get Quote', eyebrow: 'B2B Pricing', heading: 'Factory Direct Quote.', description: 'Request a POXIOL B2B quotation for basketball, soccer and multi-sport teamwear after the product, quantity, customization and destination are reviewed.', seoTitle: 'Get a Factory Quote | POXIOL Custom Teamwear Wholesale', metaDescription: 'Request a wholesale quotation for custom basketball uniforms, soccer kits, or OEM sportswear. Pricing and production plans are confirmed after the project requirements are reviewed.'},
}
...
function pageSections(key: string, title: string): CmsPageSection[] {
  ...
  const pages: Record<string, CmsPageSection[]> = {
    homepage: [
      {type: 'stats', eyebrow: 'Factory proof', title: 'Supplier strength buyers can verify', body: 'A practical teamwear supply system for clubs, schools and sportswear brands.', stats: [{value: 'Project', label: 'Project-based order planning'}, {value: 'Review', label: 'Sample availability confirmation'}, {value: 'OEM/ODM', label: 'Private-label and teamwear programs'}, {value: 'Global', label: 'Export support for B2B buyers'}]},
    ...
    about: [
      {type: 'imageText', eyebrow: 'About POXIOL', title: 'Project support for custom teamwear buyers', body: 'POXIOL helps teams and sportswear brands coordinate design, sampling, production and delivery while keeping quality details documented.', image: {url: '/images/poxiol-v62/home_hero_v62_desktop.webp', alt: 'POXIOL custom teamwear presentation'}, facts: ['B2B teamwear focus', 'Design-to-production coordination', 'Project-specific supply planning']},
    ...
    factory: [
      ...
      {type: 'stats', title: 'Factory parameters', stats: [{value: 'Project', label: 'Order structure confirmed for the project'}, {value: 'Review', label: 'Sample timing confirmed after review'}, {value: 'QC', label: 'Inspection before shipment'}, {value: 'OEM', label: 'Private-label support'}]},
    ],
    manufacturing: [
    ...
    customization: [
      ...
      {type: 'richText', title: 'Design before production', body: 'Buyers can start with a free mockup, confirm key details and then move into sample or bulk production.', cta: {label: 'Request free mockup', href: '/free-mockup/'}},
    ],
    'oem-odm': [
    ...
    'free-mockup': [
      {type: 'richText', eyebrow: 'Design preview', title: 'Request Free Mockup', body: 'Upload your team logo, reference design or color direction. POXIOL prepares a high-fidelity visual preview for review before sample or bulk production.'},
    ...
    'get-quote': [
      ...
      {type: 'faq', eyebrow: 'Buying Guide', title: 'Quote Sourcing FAQ', faqs: [{question: 'How long does it take to get a teamwear quote?', answer: 'POXIOL provides professional B2B quotes after the complete project details are reviewed.'}, {question: 'Does the quote include international shipping?', answer: 'Yes. POXIOL can include door-to-door shipping estimates in the formal quotation.'}]},
    ],
    contact: [
...
export const legacyPages: CmsPage[] = [
  {
    ...
    seo: {
      ...
      description: 'B2B custom teamwear manufacturer offering basketball uniforms, soccer kits and OEM sportswear with project-specific mockup, sample and production planning support.',
      canonicalUrl: 'https://www.poxiol.com/',
    },
  },
  ...[
    ['about', 'About POXIOL', 'B2B custom teamwear support', 'Review POXIOL’s teamwear focus, project workflow and inquiry channels.'],
    ['factory', 'Factory', 'Manufacturing visibility', 'See how custom teamwear projects move from confirmed specifications through production preparation, inspection and shipping planning.'],
    ['manufacturing', 'Manufacturing', 'OEM and ODM production', 'Review the design, sample, material, production and shipping stages used to organize a custom teamwear project.'],
    ['quality-control-process', 'Quality Control Process', 'Inspection before shipment', 'Review the checkpoints used to compare customization, sizing and packing details with the confirmed project specification.'],
    ['customization', 'Customization', 'Teamwear customization', 'Explore options for colors, logos, names, numbers, labels, packaging and product construction.'],
    ['contact', 'Contact', 'Talk to POXIOL', 'Ask a general question or choose the inquiry path that matches your current buying stage.'],
    ['oem-odm', 'OEM/ODM', 'Private-label production', 'Review OEM and ODM paths for teamwear programs, private-label ranges and project-specific customization.'],
    ['free-mockup', 'Free Mockup', 'Design before production', 'Share your design references and project requirements for an initial mockup review.'],
    ['sample-order', 'Sample Order', 'Physical sample support', 'Share the product, customization and project context needed to plan a physical sample.'],
    ['get-quote', 'Get Quote', 'Factory quote request', 'Share product, quantity, customization, destination and target date details for a project-specific quotation review.'],
  ].map(([key, title, eyebrow, description]) => ({
    key,
    slug: key,
    title,
    eyebrow,
