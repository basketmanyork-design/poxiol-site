...
test('renders the approved global-buyer content with distinct existing inquiry targets', () => {
  ...
  assert.match(text, /POXIOL supports teamwear distributors, dealers, sportswear brands, and custom resellers/)
  assert.match(text, /Design Accuracy.*Size & Fit.*Project Deadline.*Sample-to-Bulk.*Reorder Consistency.*Account Expansion/)
  const fabricLinks = anchors.filter(anchor => anchor.href === '/customization/fabric-options/')
  assert.equal(fabricLinks.length, 1, 'Buyers must have one direct homepage fabric-library entry')
  assert.equal(fabricLinks[0].children, 'Explore Fabric Library')
  assert.equal(nodes(tree).filter(node => node.type === 'section').length, 7, 'The approved compact homepage must not repeat audience, capability and approval sections')
  const process = nodes(tree).find(node => node.type === 'ol')
  assert.equal(nodes(process).filter(node => node.type === 'li').length, 5, 'Keep all five project steps in the compact process')
  assert.doesNotMatch(text, /Approval checklist explanation|Sample and bulk comparison explanation/)
