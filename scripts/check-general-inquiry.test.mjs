...
test('empty, whitespace-only, malformed and oversized inputs are rejected before a request', async () => {
  ...
  for (const change of invalid) {
    let requested = false
    await assert.rejects(async () => submit({...fields, ...change}, context, async () => { requested = true; return new Response('{"ok":true}') }), /Please enter|too long|characters/)
...
test('a filled honeypot does not submit or claim success', async () => {
  let requested = false
  await assert.rejects(async () => submit({...fields, _gotcha: 'spam'}, context, async () => { requested = true; return new Response('{"ok":true}') }), /Please use email or WhatsApp/)
  assert.equal(requested, false)
})
...
test('provider errors distinguish explicit rejection from an uncertain result', async () => {
  for (const [status, unconfirmed] of [[400,false], [422,false], [429,false], [408,true], [500,true], [503,true]]) {
    await assert.rejects(async () => submit(fields, context, async () => new Response('{"ok":true}', {status})), error => {
...
test('missing configuration cannot post to the current page', async () => {
  let requested = false
  await assert.rejects(async () => submit(fields, {...context, endpoint: undefined}, async () => { requested = true; return new Response('{"ok":true}') }), /not configured/)
  assert.equal(requested, false)
})
