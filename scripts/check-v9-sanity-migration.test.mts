...
const procurement = plan.patches.find((patch) => patch.id === 'procurementStandards')
assert.ok(procurement)
assert.equal(procurement.ifRevisionID, 'rev-procurement-1')
...
assert.equal((procurement.set.quantityPolicy as {replacement: string}).replacement, 'MOQ confirmed by product and project requirements.')
assert.equal(
...
const faq = plan.patches.find((patch) => patch.id === 'faq-1')
assert.ok(faq)
...
const project = plan.patches.find((patch) => patch.id === 'case-1')
assert.ok(project)
...
const primarySoccer = plan.patches.find((patch) => patch.id === 'cat-soccer-primary')
...
const duplicateSoccer = plan.patches.find((patch) => patch.id === 'cat-soccer-duplicate')
...
const soccerProduct = plan.patches.find((patch) => patch.id === 'product-soccer-fixture')
...
const realSnapshot = readFileSync('docs/v9-1/sanity-before.ndjson', 'utf8')
  .split(/\r?\n/)
  .filter(Boolean)
