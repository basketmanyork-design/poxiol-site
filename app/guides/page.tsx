...
export const metadata: Metadata = {
  ...
  description: "Access our CMS-managed library of teamwear buying guides with legacy fallback.",
};
    ...
    <main className="bg-[#0A0A0A] text-white selection:bg-[#B6FF00] selection:text-black">
      ...
      <section className="bg-neutral-950 px-5 py-20 md:px-10 md:py-28 xl:px-20">
        <div className="mx-auto max-w-7xl">
          <SectionHeading eyebrow="Knowledge Base" title="Professional Buying Guides" subtitle="Guides are managed in CMS, including titles, SEO, intro copy, ordering and publish status." dark center />
          <div className="mt-20 grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            {guides.map((guide) => (
              <Link key={guide.slug} href={`/guides/${guide.slug}/`} className="group rounded-3xl border border-white/10 bg-white/5 p-8 transition hover:border-lime-400/30">
                ...
                <div className="mt-8 flex items-center text-xs font-black uppercase tracking-widest text-white/40 group-hover:text-white">Read Guide <span className="ml-2 transition-transform group-hover:translate-x-1">→</span></div>
              </Link>
            ))}
          </div>
        </div>
