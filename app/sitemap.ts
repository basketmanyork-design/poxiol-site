...
import { getRouteLastModified } from "@/lib/sitemap-freshness";
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const staticPages = [
    ...
    "/solutions/",
    "/products/",
    "/products/running-track-uniforms/",
    "/products/warm-up-wear/",
  ...
  const projectRoutes = caseStudies.map((project) => ({
    ...
    lastModified: getRouteLastModified(`/projects/${project.slug}/`),
    changeFrequency: "weekly" as const,
    priority: 0.7,
  }));
  ...
  const pseoRoutes = pseoPages.map((page) => ({
    ...
    lastModified: getRouteLastModified(`/${page.slug}/`),
    changeFrequency: "weekly" as const,
    priority: 0.6,
  }));
  ...
  const cmsArticleRoutes = articles.filter((article) => isArticleRouteReleased(article.articleType, article.slug)).map((article) => {
    ...
    return {
      url: `${baseUrl}${route}`,
      lastModified: getRouteLastModified(route, article.updatedAt ? new Date(article.updatedAt) : undefined),
      changeFrequency: "weekly" as const,
