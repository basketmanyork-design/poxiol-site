...
import FreeMockupForm from "@/components/forms/FreeMockupForm";
...
export default function FreeMockupPage() {
  return (
    <main>
      ...
      <section className="bg-neutral-100 px-5 py-20 md:px-10 md:py-28 xl:px-20"><div className="mx-auto max-w-7xl"><SectionHeading eyebrow="How It Works" title="How the Free Mockup Process Works" subtitle="A simple process to turn your idea into a professional teamwear preview."/><div className="grid gap-5 md:grid-cols-4">{["Submit project details", "POXIOL reviews your request", "Design team prepares direction", "You confirm sample or quote"].map((step,index) => <div key={step} className="rounded-[1.75rem] bg-white p-7 shadow-sm"><div className="flex h-12 w-12 items-center justify-center rounded-full bg-neutral-950 text-sm font-black text-lime-400">{String(index+1).padStart(2,"0")}</div><h3 className="mt-5 text-xl font-black text-neutral-950">{step}</h3></div>)}</div></div></section>
      ...
      <section className="bg-neutral-100 px-5 py-20 md:px-10 md:py-28 xl:px-20"><div className="mx-auto max-w-4xl"><SectionHeading eyebrow="FAQ" title="Free Mockup FAQ" subtitle="Common questions about POXIOL mockup requests and next steps."/><div className="space-y-4">{freeMockupFaqs.map(faq => <details key={faq.question} className="rounded-3xl border border-neutral-200 bg-white p-6"><summary className="cursor-pointer text-lg font-black text-neutral-950">{faq.question}</summary><p className="mt-4 leading-7 text-neutral-600">{faq.answer}</p></details>)}</div></div></section>

      <section className="bg-neutral-950 px-5 py-20 md:px-10 md:py-24 xl:px-20"><div className="mx-auto max-w-7xl rounded-[2rem] border border-white/10 bg-[radial-gradient(circle_at_80%_50%,rgba(182,255,0,0.16),transparent_28%),linear-gradient(135deg,#111,#050505)] p-8 text-center md:p-14"><h2 className="text-4xl font-black leading-[1.05] text-white md:text-6xl">Ready to Start?</h2><p className="mx-auto mt-5 max-w-2xl text-lg leading-8 text-neutral-300">Submit your project details above and POXIOL will review your teamwear request.</p><Link href="#top" className="mt-8 inline-flex h-[52px] items-center justify-center rounded-full bg-lime-400 px-7 text-sm font-black uppercase tracking-wide text-neutral-950 transition hover:bg-white">Back to Form</Link></div></section>
      <Footer />
    </main>
  );
}
